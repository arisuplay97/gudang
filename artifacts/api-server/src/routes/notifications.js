...
import { db, itemsTable, materialTrackingTable, installationEvidenceTable, } from "@workspace/db";
...
router.get("/notifications", requireAuth, async (_req, res) => {
    ...
    try {
        ...
        const warningTracking = await db
            .select({ count: sql `count(*)` })
            .from(materialTrackingTable)
            .where(and(lt(materialTrackingTable.createdAt, warningStart), gte(materialTrackingTable.createdAt, slaDeadline), sql `${materialTrackingTable.status} NOT IN ('TERVERIFIKASI', 'SELESAI')`));
