...
router.get("/mutations", requireAuth, async (req, res) => {
    ...
    const conditions = [];
    if (status)
        conditions.push(eq(mutationsTable.status, status));
    if (startDate)
        conditions.push(gte(mutationsTable.transactionDate, new Date(startDate)));
    if (endDate)
        conditions.push(lte(mutationsTable.transactionDate, new Date(endDate)));
    if (search)
        conditions.push(ilike(mutationsTable.referenceNo, `%${search}%`));
    const whereClause = conditions.length > 0 ? and(...conditions) : undefined;
    const [{ count }] = await db.select({ count: sql `count(*)` }).from(mutationsTable).where(whereClause);
    const rows = await db
        .select({
    ...
    const result = await Promise.all(rows.map(async (row) => {
        ...
        const [itemCount] = await db.select({ count: sql `count(*)` }).from(mutationItemsTable).where(eq(mutationItemsTable.mutationId, row.id));
        return {
            ...row,
...
// ─── VOID ───
router.post("/mutations/:id/void", requireAuth, async (req, res) => {
    const id = parseInt(req.params.id);
    if (isNaN(id)) {
        res.status(400).json({ error: "ID tidak valid" });
        return;
