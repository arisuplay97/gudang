...
// ─── LIST ───
router.get("/stock-in", requireAuth, async (req, res): Promise<void> => {
  ...
  if (status) conditions.push(eq(stockInTable.status, status as string));
  if (supplierId) conditions.push(eq(stockInTable.supplierId, parseInt(supplierId as string)));
  if (warehouseId) conditions.push(eq(stockInTable.warehouseId, parseInt(warehouseId as string)));
  if (startDate) conditions.push(gte(stockInTable.transactionDate, new Date(startDate as string)));
  if (endDate) conditions.push(lte(stockInTable.transactionDate, new Date(endDate as string)));
  if (search) conditions.push(ilike(stockInTable.referenceNo, `%${search}%`));

  const whereClause = conditions.length > 0 ? and(...conditions) : undefined;

  // Count total
  const [{ count }] = await db.select({ count: sql<number>`count(*)` }).from(stockInTable).where(whereClause);
  ...
  const result = await Promise.all(rows.map(async (row) => {
    const [itemCount] = await db.select({ count: sql<number>`count(*)` }).from(stockInItemsTable).where(eq(stockInItemsTable.stockInId, row.id));
    return {
      ...row,
...
// ─── CREATE ───
router.post("/stock-in", requireAuth, async (req, res): Promise<void> => {
...
// ─── GET DETAIL ───
router.get("/stock-in/:id", requireAuth, async (req, res): Promise<void> => {
...
router.post("/stock-in/:id/finalize", requireAuth, async (req, res): Promise<void> => {
  const id = parseInt(req.params.id);
  if (isNaN(id)) { res.status(400).json({ error: "ID tidak valid" }); return; }
  ...
  if (header.status === "void") { res.status(400).json({ error: "Transaksi sudah dibatalkan" }); return; }
...
// ─── VOID (reverse stock) ───
router.post("/stock-in/:id/void", requireAuth, async (req, res): Promise<void> => {
  const id = parseInt(req.params.id);
  if (isNaN(id)) { res.status(400).json({ error: "ID tidak valid" }); return; }
