...
router.get("/audit-logs", requireAuth, async (req, res) => {
    const { userId, entityType, action, from, to, limit = "100" } = req.query;
    let query = db
        .select({
    ...
    if (from)
        filtered = filtered.filter(r => r.createdAt >= new Date(from));
    if (to)
        filtered = filtered.filter(r => r.createdAt <= new Date(to));
