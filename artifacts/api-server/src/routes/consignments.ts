...
// ─── LIST ───
router.get("/consignments", requireAuth, async (req, res): Promise<void> => {
    ...
    if (status) conditions.push(eq(consignmentsTable.status, status as string));
    if (search) conditions.push(ilike(consignmentsTable.referenceNo, `%${search}%`));
    const whereClause = conditions.length > 0 ? and(...conditions) : undefined;

    const [{ count }] = await db.select({ count: sql<number>`count(*)` }).from(consignmentsTable).where(whereClause);
...
// ─── CREATE ───
router.post("/consignments", requireAuth, async (req, res): Promise<void> => {
    const { itemId, quantity, unitPrice, supplierId, warehouseId, receivedDate, expiryDate, notes } = req.body;

    if (!itemId || !quantity || quantity <= 0) { res.status(400).json({ error: "Item dan quantity wajib diisi" }); return; }
    if (!receivedDate) { res.status(400).json({ error: "Tanggal terima wajib diisi" }); return; }
...
// ─── GET DETAIL ───
router.get("/consignments/:id", requireAuth, async (req, res): Promise<void> => {
...
// ─── UPDATE STATUS ───
router.patch("/consignments/:id", requireAuth, async (req, res): Promise<void> => {
    const id = parseInt(req.params.id);
    if (isNaN(id)) { res.status(400).json({ error: "ID tidak valid" }); return; }
...
// ─── DELETE ───
router.delete("/consignments/:id", requireAuth, async (req, res): Promise<void> => {
    const id = parseInt(req.params.id);
    if (isNaN(id)) { res.status(400).json({ error: "ID tidak valid" }); return; }
