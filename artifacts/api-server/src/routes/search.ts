...
router.get("/search", requireAuth, async (req, res): Promise<void> => {
    ...
    const pattern = `%${q}%`;
    const role = (req.session as any).role || "GUDANG";
    ...
    try {
        ...
        const materials = await db
            ...
            .where(
                or(
                    ...
                    ilike(itemsTable.code, pattern),
                    ilike(itemsTable.barcode, pattern),
                )
            )
            .limit(5);
        ...
        const branches = await db
            ...
            .where(
                or(
                    ilike(branchesTable.name, pattern),
                    ilike(branchesTable.code, pattern),
                )
            )
            .limit(5);
