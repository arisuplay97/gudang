...
router.get("/branch/shipments", requireAuth, requireRole("CABANG", "ADMIN"), async (req, res) => {
...
router.get("/branch/shipment/:tokenOrId", requireAuth, requireRole("CABANG", "ADMIN"), async (req, res) => {
    ...
    const [shipment] = await db
        ...
        .leftJoin(branchesTable, eq(stockOutTable.destinationBranchId, branchesTable.id))
        .leftJoin(warehousesTable, eq(stockOutTable.warehouseId, warehousesTable.id))
        .where(or(eq(stockOutTable.qrToken, param), eq(stockOutTable.referenceNo, param), ...(isNum ? [eq(stockOutTable.id, parseInt(param))] : [])));
    if (!shipment) {
...
router.post("/branch/receive-unit", requireAuth, requireRole("CABANG", "ADMIN"), async (req, res) => {
    ...
    await db.transaction(async (tx) => {
        ...
        if (remainingUnits === 0) {
            const [existingReceipt] = await tx.select().from(materialReceiptsTable)
                .where(and(eq(materialReceiptsTable.transactionId, tracking.stockOutId), eq(materialReceiptsTable.branchId, userBranchId)));
            if (!existingReceipt) {
...
router.get("/branch/dashboard-stats", requireAuth, requireRole("CABANG", "ADMIN"), async (req, res) => {
    let branchId = req.session.branchId;
    if (!branchId && req.session.userRole === "ADMIN") {
        if (req.query.branchId)
            branchId = parseInt(req.query.branchId);
    ...
    const trkConditions = [];
    if (branchId)
        trkConditions.push(eq(materialTrackingTable.branchId, branchId));
    ...
    const soConditions = [eq(stockOutTable.status, "DIKIRIM")];
    if (branchId)
        soConditions.push(eq(stockOutTable.destinationBranchId, branchId));
...
router.post("/branch/receive", requireAuth, requireRole("CABANG", "ADMIN"), async (req, res) => {
    const { qrToken, idempotencyKey } = req.body;
    if (!qrToken) {
        res.status(400).json({ error: "QR token atau nomor Surat Jalan wajib diisi" });
        return;
    }
    ...
    const [transaction] = await db.select().from(stockOutTable)
        .where(or(eq(stockOutTable.qrToken, cleanToken), eq(stockOutTable.referenceNo, cleanToken), eq(stockOutTable.referenceNo, cleanToken.replace(/^QR-/, "")), ...(isNum ? [eq(stockOutTable.id, parseInt(cleanToken))] : [])));
    if (!transaction) {
...
router.post("/branch/allocations", requireAuth, requireRole("CABANG", "ADMIN"), async (req, res) => {
    ...
    if (!trackingUuid || !resolvedQty || resolvedQty <= 0) {
        res.status(400).json({ error: "trackingUuid dan quantity valid wajib diisi" });
        return;
    }
...
router.post("/branch/evidence", requireAuth, requireRole("CABANG", "ADMIN"), async (req, res) => {
    ...
    const districtAudit = auditCrossDistrictEvidence(branchName, parseFloat(String(latitude)), parseFloat(String(longitude)));
...
router.get("/branch/my-allocations", requireAuth, requireRole("CABANG", "ADMIN"), async (req, res) => {
    let branchId = req.session.branchId;
    if (!branchId && req.session.userRole === "ADMIN") {
        if (req.query.branchId)
            branchId = parseInt(req.query.branchId);
    ...
    if (branchId) {
        query = query.where(eq(materialTrackingTable.branchId, branchId));
    }
    const rows = await query.orderBy(desc(installationAllocationsTable.createdAt));
    res.json({ data: rows });
});
/**
