...
// ─── LIST ───
router.get("/tool-loans", requireAuth, async (req, res): Promise<void> => {
    ...
    if (status) conditions.push(eq(toolLoansTable.status, status as string));
    if (search) conditions.push(ilike(toolLoansTable.referenceNo, `%${search}%`));
    const whereClause = conditions.length > 0 ? and(...conditions) : undefined;

    const [{ count }] = await db.select({ count: sql<number>`count(*)` }).from(toolLoansTable).where(whereClause);
...
// ─── CREATE ───
router.post("/tool-loans", requireAuth, async (req, res): Promise<void> => {
    const { borrowerName, departmentId, warehouseId, loanDate, expectedReturnDate, notes, items } = req.body;

    if (!borrowerName) { res.status(400).json({ error: "Nama peminjam wajib diisi" }); return; }
    if (!loanDate || !expectedReturnDate) { res.status(400).json({ error: "Tanggal pinjam dan rencana kembali wajib diisi" }); return; }
    if (!items || !Array.isArray(items) || items.length === 0) { res.status(400).json({ error: "Minimal 1 item harus diisi" }); return; }
    if (!warehouseId) { res.status(400).json({ error: "Gudang wajib dipilih" }); return; }
...
// ─── GET DETAIL ───
router.get("/tool-loans/:id", requireAuth, async (req, res): Promise<void> => {
...
// ─── RETURN ITEMS ───
router.post("/tool-loans/:id/return", requireAuth, async (req, res): Promise<void> => {
    const id = parseInt(req.params.id);
    if (isNaN(id)) { res.status(400).json({ error: "ID tidak valid" }); return; }

    const { returnItems } = req.body; // Array of { toolLoanItemId, returnQuantity, condition }
    if (!returnItems || !Array.isArray(returnItems) || returnItems.length === 0) {
        res.status(400).json({ error: "Data pengembalian wajib diisi" }); return;
    }

    const [header] = await db.select().from(toolLoansTable).where(eq(toolLoansTable.id, id));
    if (!header) { res.status(404).json({ error: "Tidak ditemukan" }); return; }
    if (header.status === "returned") { res.status(400).json({ error: "Sudah dikembalikan semua" }); return; }

    const warehouseId = header.warehouseId;
    if (!warehouseId) { res.status(400).json({ error: "Gudang tidak valid" }); return; }
